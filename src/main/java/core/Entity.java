...
public class Entity implements Cloneable {
	...
	Object obj; // stores custom objects. It can Party, Attorney, Judge, etc..
	...
	public static void resetSerial() {
		globalIDSource.set(savedGlobalid);
	}

	public static void getGlobalid() {
		savedGlobalid = globalIDSource.get() + 1;
	}
	...
	public boolean isKindOf(Entity cls) {
		...
		if (theClass != null) {
			if (onto.isKindOf(theClass.name, cls.getName())) {
				return true;
			}
			if (theClass.theClass != null) {
				if (onto.isKindOf(theClass.theClass.name, cls.getName())) {
					return true;
				}
			}
		}
	...
	public boolean isKindOf_Old(Entity cls) {
		...
		for (Link lk : onto.relations) {
			...
			if (this.equals(e1) || (theClass != null && theClass.equals(e1))
					|| (theClass != null && theClass.theClass != null && theClass.theClass.equals(e1))) {
